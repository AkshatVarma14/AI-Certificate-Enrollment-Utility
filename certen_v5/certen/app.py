...
def admin_dashboard():
    ...
    user_task_rows         = []
    # cert for the selected user+program combo (user-centric view)
    ...
    if selected_user:
        ...
        if selected_program:
            ...
            user_program_cert = Certificate.query.filter_by(
                user_id=selected_user.id, program_id=selected_program.id
            ).first()
    ...
    program_detail_cert    = None   # cert for prog_detail_user in this program
    ...
    if selected_program and view_mode == "program":
        ...
        if prog_detail_user_id:
            ...
            if program_detail_user:
                ...
                program_detail_cert = Certificate.query.filter_by(
                    user_id=prog_detail_user_id, program_id=selected_program.id
                ).first()
...
def grant_certificate():
    ...
    # Return to the correct view mode
...
@app.route("/certificate/view/<int:user_id>/<int:program_id>")
def view_certificate(user_id, program_id):
    """Render a printable certificate page."""
    cert    = Certificate.query.filter_by(user_id=user_id, program_id=program_id,
                                          status="granted").first_or_404()
